...
export function createAdminService(store, events) {
  ...
  async function loadRedemptionCodes() {
    ...
    if (error) {
      ...
      events.emit('supabase:error', { scope: 'admin', action: 'loadRedemptionCodes', error });
      throw error;
    }
  ...
  async function createRedemptionCode(payload) {
    ...
    if (error) {
      ...
      events.emit('supabase:error', { scope: 'admin', action: 'createRedemptionCode', error });
      throw error;
    }
  ...
  async function deactivateCode(codeId) {
    ...
    if (error) {
      ...
      events.emit('supabase:error', { scope: 'admin', action: 'deactivateCode', error });
      throw error;
    }
    await loadRedemptionCodes();
    return data;
