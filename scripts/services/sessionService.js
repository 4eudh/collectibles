...
export function createSessionService(store, events) {
  ...
  async function signIn(email, password) {
    ...
    if (error) {
      events.emit('supabase:error', { scope: 'session', action: 'signIn', error });
      throw error;
    }
  ...
  async function signUp(email, password, metadata = {}) {
    ...
    if (error) {
      events.emit('supabase:error', { scope: 'session', action: 'signUp', error });
      throw error;
    }
  ...
  async function signOut() {
    ...
    if (error) {
      events.emit('supabase:error', { scope: 'session', action: 'signOut', error });
      throw error;
    }
    store.reset();
