...
export function createEconomyService(store, events) {
  ...
  async function loadWallet(userId) {
    ...
    if (error && error.code !== 'PGRST116') {
      ...
      events.emit('supabase:error', { scope: 'economy', action: 'loadWallet', error });
      throw error;
    }
    if (!data) {
      ...
      if (createError) {
        ...
        events.emit('supabase:error', { scope: 'economy', action: 'createWallet', error: createError });
        throw createError;
      }
  ...
  async function loadLedger(userId, limit = 25) {
    ...
    if (error) {
      ...
      events.emit('supabase:error', { scope: 'economy', action: 'loadLedger', error });
      throw error;
    }
  ...
  async function recordLedgerEntry(entry) {
    ...
    if (error) {
      ...
      events.emit('supabase:error', { scope: 'economy', action: 'recordLedgerEntry', error });
      throw error;
    }
  ...
  async function grantDailyStipend(userId, wallet) {
    ...
    if (error) {
      ...
      events.emit('supabase:error', { scope: 'economy', action: 'grantDailyStipend', error });
      throw error;
    }
  ...
  async function adjustBalance(userId, deltas, context) {
    ...
    if (error) {
      ...
      events.emit('supabase:error', { scope: 'economy', action: 'adjustBalance', error });
      throw error;
    }
    await recordLedgerEntry({
      user_id: userId,
